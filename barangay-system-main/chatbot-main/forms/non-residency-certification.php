        ...
        try {
            ...
            if ($hasUserId) {
                $stmt = $pdo->prepare("INSERT INTO non_residency_certification_requests 
                    (full_name, previous_address, purpose, shipping_method, cost, user_id)
                    VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([
                    $full_name, $previous_address, $purpose, $shipping_method, $cost, $_SESSION['user_id']
                ]);
            } else {
                $stmt = $pdo->prepare("INSERT INTO non_residency_certification_requests 
                    (full_name, previous_address, purpose, shipping_method, cost)
                    VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([
                    $full_name, $previous_address, $purpose, $shipping_method, $cost
                ]);
...
<nav class="main-nav" id="mainNav">
  ...
  <a href="../profile.php"><i class="fas fa-user"></i> My Profile</a>
    ...
    <!-- Cover Photo -->
    <div class="header-banner">Non-Residency Certificate Form</div>
